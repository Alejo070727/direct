...
if __name__ == '__main__':
    ...
    # MANTENER AL FILETOLINK DESPIERTO #
    try:
        for port in range(1,99999999999999999999999): #Si, exageré XD
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            result = sock.connect_ex(('127.0.0.1',port))
            print('No tengo sueño, soy pro :D')
            if result == 0:
                print('Encontré un puerto pero no interesa, seguiré despierto xD')
            else:
                sock.close()
    except:
        print('Tengo sueño, voy a dormir...')